...
RESULTS_DIR = "results_residual_attention"
...
class ResidualAttentionFusionModel(nn.Module):
    ...
    def forward(self, swin_feat, lesion_feat):
        ...
        weighted_swin = swin_feat * (1 + alpha)  # Residual gating
...
plt.plot([0,1],[0,1],'k--')
plt.title("ROC - Residual Attention")
plt.legend()
plt.tight_layout()
...
plt.title("PR - Residual Attention")
plt.legend()
plt.tight_layout()
...
print("\nResidual Attention Fusion complete.")
